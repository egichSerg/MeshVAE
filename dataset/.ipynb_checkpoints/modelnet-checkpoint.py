...
class ModelNetRenders(torch.utils.data.Dataset):
  ...
  # 6. Overwrite __getitem__()
  def __getitem__(self, index: int) -> Tuple[torch.Tensor, int]:
      renders = torch.from_numpy(np.load(self.dataset_dir / self.data_table.iloc[index].object_path.strip(' '))).unsqueeze(1)
      label = self.class_to_idx[self.data_table.iloc[index]['class']]

      if self.transform:
          return self.transform(renders), label
      
      return renders, label
