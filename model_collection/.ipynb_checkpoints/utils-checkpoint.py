...
def max_VRAM_optim_train_step(model, target_cameras, render_bs, optimizer, criterion, dataloader, device, scheduler, **kwargs):
    model.train()
...
def max_VRAM_optim_eval_step(model, target_cameras, render_bs, criterion, dataloader, device, **kwargs):
    model.eval()
    avg_loss = 0.
...
def max_VRAM_gan_train_step(model, target_cameras, render_bs, optimizer, criterion, dataloader, device, scheduler, **kwargs):
    model.train()
    encoder_modules = model.get_encoder_modules()
    decoder_modules = model.get_decoder_modules()
...
def max_VRAM_optim_fit(model, optimizer, criterion, train_dl, test_dl, device, epochs, train_launch, scheduler=None, mode='vae',
    ...
    if mode=='vae':
        train_step = max_VRAM_optim_train_step
        eval_step = max_VRAM_optim_eval_step
    elif mode=='gan':
        train_step = max_VRAM_gan_train_step
        eval_step = max_VRAM_gan_train_step
    ...
    for epoch in range(epochs):
        ...
        print('Training...')
        train_loss = train_step(model, target_cameras, render_batching_size, optimizer, criterion, train_dl, device, scheduler)        
        
        if mode == 'gan':
            test_loss = train_loss
        else:
            print('Validating...')
            test_loss = eval_step(model, target_cameras, render_batching_size, criterion, test_dl, device)
