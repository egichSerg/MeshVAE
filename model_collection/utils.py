...
from model_collection.MeshVAE import VAELoss
...
# TODO: скопипастить процесс обучения из FittingVolumes, мейби даже FittingNeRF
def max_VRAM_optim_train_step(model, target_cameras, render_bs, optimizer, criterion, dataloader, device, scheduler):
    ...
    for input_renders in tqdm(dataloader):
        ...
        #make random renders
        batch = batch_idx = torch.randperm(40)[:batch_size]
        ...
        loss = criterion(mu, logvar, rendered_images, rendered_silhouttes, target_renders, target_silhouttes)
        ...
        # print('avg loss over batch', avg_loss / len(dataloader))
...
def max_VRAM_optim_fit(model, optimizer, criterion, train_dl, test_dl, device, epochs, train_launch, scheduler=None,
                       ...
                       restore_best_weights=True, save_loss_history=True):
